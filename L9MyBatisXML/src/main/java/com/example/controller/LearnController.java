...
public class LearnController {
    ...
    public void queryLearnList(HttpServletRequest request , HttpServletResponse response){
        ...
        List<Course> learnList= courseService.queryLearnResouceList(params);
    ...
    public void addLearn(HttpServletRequest request , HttpServletResponse response){
        ...
        course.setUrl(url);
        int index= courseService.add(course);
        if(index>0){
    ...
    public void updateLearn(HttpServletRequest request , HttpServletResponse response){
        ...
        Course course = courseService.queryLearnResouceById(Long.valueOf(id));
        String author = request.getParameter("author");
        String title = request.getParameter("title");
        String url = request.getParameter("url");
        if(StringUtil.isNull(author)){
        ...
        course.setTitle(title);
        course.setUrl(url);
        int index= courseService.update(course);
    ...
    public void deleteUser(HttpServletRequest request , HttpServletResponse response){
        ...
        JSONObject result = new JSONObject();
        //删除操作
        int index = courseService.deleteByIds(ids.split(","));
        if(index>0){
