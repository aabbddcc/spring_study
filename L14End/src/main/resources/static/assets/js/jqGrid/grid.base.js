...
//module begin
        ...
        return this.each(function () {
                ...
                populateVisible: function () {
                    ...
                    if (npage) {
                        ...
                        if (p.scrollPopUp && p.lastpage != null) {
                            $("#scroll_g" + p.id).show().html($.jgrid.template($.jgrid.getRegional(ts, "defaults.pgtext", p.pgtext), p.page, p.lastpage)).css({
            ...
            var sortkeys = ["shiftKey", "altKey", "ctrlKey"],
                ...
                addXmlData = function (xml, rcnt, more, adjust) {
                    ...
                    var self = $(ts), i, fpos, ir = 0, v, gi = ts.p.multiselect === true ? 1 : 0, si = 0,
                        addSubGridCell, ni = ts.p.rownumbers === true ? 1 : 0, idn, getId, f = [], F, rd = {},
                ...
                addJSONData = function (data, rcnt, more, adjust) {
                    ...
                    var self = $(ts), ir = 0, v, i, j, f = [], cur, gi = ts.p.multiselect ? 1 : 0,
                        ...
                        rd = {}, fpos, idr, rowData = [], cn = (ts.p.altRows === true) ? ts.p.altclass : "",
                        cn1 = getstyle(stylemodule, 'rowBox', true, 'jqgrow ui-row-' + ts.p.direction), classes;
                ...
                addLocalData = function () {
                    var st = ts.p.multiSort ? [] : "", sto = [], fndsort = false, cmtypes = {}, grtypes = [],
                        grindexes = [], srcformat, sorttype, newformat, sfld;
                    ...
                    if (pgboxes) {
                        ...
                        if (ts.p.pgbuttons === true) {
                            ...
                            if (cp === 1 || cp === 0) {
                                ...
                                if (ts.p.toppager) {
                                    $("#first_t" + tspg_t + ", #prev_t" + tspg_t).addClass(disabled).removeClass(hover);
                                }
                            } else {
                                $("#first" + tspg + ", #prev" + tspg).removeClass(disabled);
                                if (ts.p.toppager) {
                                    $("#first_t" + tspg_t + ", #prev_t" + tspg_t).removeClass(disabled);
                                }
                            }
                            if (cp === last || cp === 0) {
                                $("#next" + tspg + ", #last" + tspg).addClass(disabled).removeClass(hover);
                                if (ts.p.toppager) {
                                    $("#next_t" + tspg_t + ", #last_t" + tspg_t).addClass(disabled).removeClass(hover);
                                }
                            } else {
                                $("#next" + tspg + ", #last" + tspg).removeClass(disabled);
                                if (ts.p.toppager) {
                                    $("#next_t" + tspg_t + ", #last_t" + tspg_t).removeClass(disabled);
                                }
                            }
                        }
                    }
			...
			*/
            ...
            if (ts.p.footerrow) {
                ...
                if (ts.p.rownumbers) {
                    grid.footers[0].className = getstyle(stylemodule, 'rownumBox', true, 'jqgrid-rownum');
                }
                if (hg) {
                    $(grid.sDiv).hide();
                }
    ...
    $.jgrid.extend({
        ...
        setSelection: function (selection, onsr, e) {
            return this.each(function () {
                ...
                if (!$t.p.multiselect) {
                    ...
                    if (ia === -1) {
                        if (pt.className !== "ui-subgrid") {
                            $(pt).addClass(highlight).attr("aria-selected", "true");
                        }
                        stat = true;
                        $t.p.selarrrow.push($t.p.selrow);
                    } else {
                        if (pt.className !== "ui-subgrid") {
                            $(pt).removeClass(highlight).attr("aria-selected", "false");
                        }
                        stat = false;
        ...
        resetSelection: function (rowid) {
            return this.each(function () {
                ...
                if (rowid !== undefined) {
                    ...
                    if (fid) {
                        $("#" + $.jgrid.jqID(sr), "#" + $.jgrid.jqID(fid)).removeClass(highlight);
                    }
                    if (t.p.multiselect) {
        ...
        addRowData: function (rowid, rdata, pos, src) {
            ...
            var success = false, nm, row, rnc = "", msc = "", gi, si, ni, sind, i, v, prp = "", aradd, cnm, cn, data,
                cm, id;
            if (rdata) {
                if ($.isArray(rdata)) {
                    aradd = true;
                ...
                this.each(function () {
                    ...
                    cn = t.p.altclass;
                    var k = 0,
                        cna = $(t).jqGrid('getStyleUI', t.p.styleUI + ".base", 'rowBox', true, 'jqgrow ui-row-' + t.p.direction),
                        lcdata = {}, classes,
